...
//           });
//           setLogoPreview(response.data.logoUrl); // Update this to store the logo URL from the server
//           setIsUpdating(true);
//         }
//       })
...
//           setLogoPreview(response.data.logoUrl); // Set the returned logo URL to display the logo
//         }
//         if (!isUpdating) {
//           setIsUpdating(true);
//         }
...
const CompanyProfile = () => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post(url, formData)
      .then((response) => {
        const { cname } = response.data.profile;
      // Store cname in local storage
      localStorage.setItem('cname', cname);
        window.alert(`${isUpdating ? 'Updated' : 'Created'} profile successfully`);
        setLogoPreview(`http://localhost:3000/${response.data.updatedProfile?.logoUrl || response.data.profile.logoUrl}`);
        if (!isUpdating) {
          setIsUpdating(true);
        }
      })
