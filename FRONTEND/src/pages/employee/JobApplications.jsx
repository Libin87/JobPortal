...
//       try {
//         const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/jobs/applications`, { params: { userId } });
...
//     try {
//       const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/profile/${employerId}`);
...
//       <Avatar
//         src={`${process.env.REACT_APP_BASE_URL}/${companyDetails.logoUrl}`}
//         alt="Company Logo"
...
const JobApplications = () => {
  ...
  const fetchCompanyDetails = async (userId) => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/profile/${userId}`);
      setCompanyDetails(response.data);
      setCompanyProfileOpen(true);
    } catch (error) {
      toast.error("Error fetching company details");
